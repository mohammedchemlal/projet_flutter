...
import 'package:flutter_1/globals.dart';
...
String us = globals.globalDataS;
class infosBasiques extends StatelessWidget {
  const infosBasiques({Key? key}) : super(key: key);

  /*Future<bool> checkIfUsernameExists(String user) async {
    Uri url = Uri.parse("http://127.0.0.1/api_flutter_1/user/ifUserExists.php");

    var response = await http.post(url,
        body: {'username': user});

    if (response.statusCode == 200) {
      // If the response is successful, check the content
      String result = response.body.trim(); // Remove any extra spaces

      // Check the response content and return true if it's "Success"
      return result == "Success";
    } else {
      // If the request fails or an error occurs, return false
      return false;
    }
  }*/
  ...
  /*Future<String> getPassword() async {
    final Uri uri = Uri.parse('http://127.0.0.1/api_flutter_1/user/retrievedata.php');

    try {
      final response = await http.get(uri);

      if (response.statusCode == 200) {
        // Return the password retrieved from the server
